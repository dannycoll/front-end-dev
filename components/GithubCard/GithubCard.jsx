...
const GithubCard = () => {
  ...
  const fetchData = async () => {
    const fetched = await fetch("https://api.github.com/users/dannycoll");
  ...
  const fetchData2 = async () => {
    const fetched = await (
      await fetch("https://api.github.com/users/dannycoll/events")
    ).json();
    const recentRepos = [
      ...fetched.map((x) => ({
        name: x.repo.name,
    ...
    const reposFetched = await (
      await fetch("https://api.github.com/users/dannycoll/repos")
    ).json();
    ...
    chartData.datasets[0].data = [
      ...chartData.labels.map(
        (x) => reposFetched.map((y) => y.lang).filter((y) => y == x).length
      ),
    ];
