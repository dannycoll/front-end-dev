...
import { animationDuration, tileCount as tileCountPerRowOrColumn } from '../components/2048Game/Board';
...
export const useGame = () => {
  ...
  useEffect(() => {
    if (isInitialRender.current) {
      const pos1 = [Math.floor(Math.random() * 3), Math.floor(Math.random() * 3)];
      let pos2 = [Math.floor(Math.random() * 3), Math.floor(Math.random() * 3)];
      while (pos2[0] === pos1[0] && pos2[1] === pos1[1]) {
        pos2 = [Math.floor(Math.random() * 3), Math.floor(Math.random() * 3)];
      }
      createTile({ position: pos1, value: 2 });
      createTile({ position: pos2, value: 2 });
